...
Default child-safe imports are `plan-mode`, `get-artifact`, `storage-explorer`, `file-organization`, `image-gen`, `git-gh`, `meta-gateway`, `omni-qa`, `orchestration`, generic `SOUL.md`, Xan preference seed memory, sync policy, and agent message contract. The canonical skill bundle lives at `Hermes/Init/defaults/core-common/` and is versioned via the `mc-init` repo (`https://github.com/KffeePt/mc-init`). Do not copy private keys, raw `.env` files, raw memory dumps, or another machine's `local-adapted`/`private` skills into child seeds.
